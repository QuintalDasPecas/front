...
export default {
    ...
    methods:{
        ...
        async handleStore(){
            ...
            const { data: responseData, error: responseError } = await subscribe.store(formData);
        ...
        async handleUpdate(){
            ...
            const { data: responseData, error: responseError } = await subscribe.update(this.formData.id,formData);
            let status = responseData.value ? responseData._rawValue.status : null;
            status = status ?? (responseError.value ? responseError.value.statusCode : null);
        ...
        async handleGetAllActive(){
            ...
            const { data: responseData, error: responseError } = await subscribe.all();
            let status = responseData.value ? responseData._rawValue.status : null;
            status = status ?? (responseError.value ? responseError.value.statusCode : null);
        ...
        async handleEnable(id){           
            ...
            const result = await subscribe.enable(id, form);
            this.handleGetAllActive();
        },
        async handleDestroy(id){ 
            const subscribe = new Subscribe();                               
            const result = await subscribe.destroy(id);
            this.handleGetAllActive();
        },
