...
export default {
    ...
    methods:{
        ...
        async handleStore(){
            ...
            const { data: responseData, error: responseError } = await payTerm.store(formData);
        ...
        async handleUpdate(){
            ...
            const { data: responseData, error: responseError } = await payTerm.update(this.formData.id,formData);
            let status = responseData.value ? responseData._rawValue.status : null;
            status = status ?? (responseError.value ? responseError.value.statusCode : null);
        ...
        async handleGetAllActive(){
            ...
            const { data: responseData, error: responseError } = await payTerm.all();
            let status = responseData.value ? responseData._rawValue.status : null;
            status = status ?? (responseError.value ? responseError.value.statusCode : null);
        ...
        async handleEnable(id){           
            ...
            const result = await payTerm.enable(id, form);
            this.handleGetAllActive();
        },
        async handleDestroy(id){ 
            const payTerm = new PayTerm();                               
            const result = await payTerm.destroy(id);
            this.handleGetAllActive();
        },
